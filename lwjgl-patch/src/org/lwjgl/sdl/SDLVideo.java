 ...
 * MACHINE GENERATED FILE, patched by HTS (see HtsGLAttributes)
 */
...
public class SDLVideo {
    ...
    // HTS: the loading screen's context sharing (HtsGLBaton) makes contexts current through SDL
    static {
        HtsGLBaton.setBackend(new HtsGLBaton.Backend() {
            @Override
            public boolean makeCurrent(long window, long context) {
                return realMakeCurrent(window, context);
            }
    ...
    public static long nSDL_CreateWindow(long title, int w, int h, long flags) {
        // HTS: Android has one window; lwjgl3ify's hidden one for shared contexts becomes virtual
        long virtual = (flags & SDL_WINDOW_HIDDEN) != 0 ? HtsGLBaton.virtualWindow() : NULL;
        if (virtual != NULL) {
            return virtual;
        }
        long __functionAddress = Functions.CreateWindow;
        long window = invokePJP(title, w, h, flags, __functionAddress);
        HtsGLBaton.onWindowCreated(window);
        return window;
    }
    ...
    public static long SDL_CreateWindowWithProperties(@NativeType("SDL_PropertiesID") int props) {
        ...
        long window = invokeP(props, __functionAddress);
        // HTS: lwjgl3ify creates the game window this way; remember it as the main one
        HtsGLBaton.onWindowCreated(window);
        return window;
    }
    ...
    public static void SDL_DestroyWindow(@NativeType("SDL_Window *") long window) {
        if (HtsGLBaton.isVirtual(window)) {
            return;
        }
    ...
    public static long nSDL_GL_GetProcAddress(long proc) {
        // HTS: GL comes from the translation layer, not from whatever the EGL driver exports
        long address = HtsGLAttributes.procAddress(proc);
        if (address != NULL) {
            return address;
        }
    ...
    public static void SDL_GL_ResetAttributes() {
        ...
        invokeV(__functionAddress);
        // HTS: pin the context type the selected Android renderer can actually create
        HtsGLAttributes.afterReset(Functions.GL_SetAttribute);
    }
    ...
    public static boolean SDL_GL_SetAttribute(@NativeType("SDL_GLAttr") int attr, int value) {
        ...
        // HTS: rewrite desktop-only requests (e.g. "2.1" without a profile) for Android EGL
        value = HtsGLAttributes.filter(attr, value);
    ...
    public static long SDL_GL_CreateContext(@NativeType("SDL_Window *") long window) {
        // HTS: a "shared" context on the virtual window is the main context itself (see HtsGLBaton)
        if (HtsGLBaton.isVirtual(window)) {
            return HtsGLBaton.virtualContext();
        }
        long __functionAddress = Functions.GL_CreateContext;
        if (CHECKS) {
            check(window);
        }
        long context = invokePP(window, __functionAddress);
        HtsGLBaton.onContextCreated(window, context, true); // SDL makes it current
        return context;
    }
    ...
    public static boolean SDL_GL_MakeCurrent(@NativeType("SDL_Window *") long window, @NativeType("SDL_GLContext") long context) {
        return HtsGLBaton.makeCurrent(window, context);
    }

    /** HTS: the real SDL_GL_MakeCurrent, without virtual handles or the splash baton. */
    static boolean realMakeCurrent(long window, long context) {
        long __functionAddress = Functions.GL_MakeCurrent;
        return invokePPZ(window, context, __functionAddress);
    }

    /** HTS: the real SDL_GL_GetCurrentContext, without virtual handles. */
    static long realCurrentContext() {
        return invokeP(Functions.GL_GetCurrentContext);
    }
    ...
    public static long SDL_GL_GetCurrentWindow() {
        ...
        return HtsGLBaton.currentWindow(invokeP(__functionAddress));
    }
    ...
    public static long SDL_GL_GetCurrentContext() {
        ...
        return HtsGLBaton.currentContext(invokeP(__functionAddress));
    }
    ...
    public static boolean SDL_GL_SetSwapInterval(int interval) {
        // HTS: vsync is paced by HtsSurface (eglSwapInterval does nothing on a TextureView). EGL
        // stays at 0 so a driver that does block can't add its wait to ours. Never fails: the
        // game sets it while loading, when its thread may have lent the context to the splash
        // (lwjgl3ify threw "No OpenGL context has been made current" and OptiFine dropped the
        // maxFps/enableVsync options).
        HtsSurface.setSwapInterval(interval);
        if (realCurrentContext() != NULL) {
            invokeZ(0, Functions.GL_SetSwapInterval);
        }
        return true;
    }
    ...
    public static boolean nSDL_GL_GetSwapInterval(long interval) {
        // HTS: the interval the game asked for, see SDL_GL_SetSwapInterval
        memPutInt(interval, HtsSurface.swapInterval());
        return true;
    }
    ...
    public static boolean SDL_GL_SwapWindow(@NativeType("SDL_Window *") long window) {
        ...
        long real = HtsGLBaton.realWindow(window);
        boolean result = invokePZ(real, __functionAddress);
        if (result) {
            HtsSurface.swapped(real);
        } else {
            result = HtsSurface.swapFailed(real);
        }
        HtsGLBaton.afterSwap();
        return result;
    }
    ...
    public static boolean SDL_GL_DestroyContext(@NativeType("SDL_GLContext") long context) {
        if (HtsGLBaton.isVirtual(context)) {
            return true;
        }
