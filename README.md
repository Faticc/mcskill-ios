...
- `.github/workflows/ios.yml` — `swift test` → сборка под симулятор → скриншоты → неподписанный `McSkill.ipa`.

## Что выдаёт CI (артефакты запуска)
- `McSkill-ipa`: ставится на iPhone через Sideloadly или AltStore/SideStore под своим Apple ID.
- `screenshots`: экран входа и список серверов в демо‑режиме (`--demo`), плюс лог приложения из симулятора.
- `logs`: вывод тестов и сборок.

## Логи с телефона
Меню «…» → «Поделиться логами», или приложение «Файлы» → На iPhone → McSkill → logs.
